...
function addTagForm($collectionHolder, $newLinkLi) {
    ...
    let $newFormLi = $('<ul id="date_' + index + '" class="date card-group d-flex flex-column"></ul>').append(newForm);
    let $div = $('<div></div>');
    $newFormLi.append($div);
    $div.append('<button type="button" class="remove-tag btn btn-danger">Supprimer la date</button>');
    $div.append('<button type="button" name="__heures__"  data="' + index + '" class="btn btn-secondary">Ajouter une plage horaire</button>');
...
function addHeure($collectionHeure, $newLinkLi, $data) {
    ...
    $('#planning_journees_' + $data + '_heures_' + index).append('<button type="button" class="remove-tag btn btn-warning">X</button>');
    $('.remove-tag').click(function() {
        $(this).parent().remove();
        return false;
    });
